...
char get_Register_out_Bin(char* RegName, char* error){
    // 레지스터 이름을 바이너리로 변환해서 반환
        ...
        strcpy(error, "Invalid Regster name.");
        return 0xFF;
}
...
bool command_syntax(char* line, uint32_t* outputBin, char* error){
    // Opcode 확인 및 추가
        ...
        if (error[0] == 'P'){
            strcpy(error, op);
            strcat(error, " is not vz32 opcode.\0");
            return 0;
        }
...
int main(/*int argc, char* argv*/){
    //open();
    char line[4][50] = {"ADD-IMM tm1, tm2, 3", "EAA i0, i1", "SUB i0, i2, i14", "ADD i99, i92, e32"};
    uint32_t outBin[4] = {0, 0};
    char error[50];
    for (char i = 0; i < 4; i++){
